...
#include "pwm.h"
static void pwm_light_init(void);

#define Magnetic_door
	...
	#define GPIO_RED 			P34
	#define GPIO_BLUE			P7
	#define DOOR_STATE 		P2
	#define res_network		P3
	#define tamper_door 	P14
	#define RESET 				P18
 
	uint8 mac_addr[6]={0}; 
	void gpio_door_init(void);
	void door_gpio_interrupt(gpio_pin_e pin);
	void res_network_event_handler(GPIO_Pin_e pin,IO_Wakeup_Pol_e type);
	static void updateAdvData_door(uint8_t id);
	void read_gpioSTATE(void);
...
void SimpleBLEPeripheral_Init( uint8 task_id )
{
    ...
    LL_PLUS_PerStats_Init(&g_perStatsByChanTest);
			...
			gpio_door_init();
			read_gpioSTATE();	
			LOG("gpio_init_state R:%d DOOR_O:%d DOOR_T:%d\r\n",advertData[13],advertData[14],advertData[15]);	
			updateAdvData_door(0);				
		#endif
...
#if 0
pwm_ch_t pwm_light_ch1;
pwm_ch_t pwm_light_ch2;
uint8_t white_value;
uint8_t yellow_value;

#define LIGHT_POWER_ON  	0x00
#define LIGHT_POWER_OFF  	0xFF
#define LIGHT_POWER_MODEL 0XFE

bool light_state = false;
void lxl_pwm_off(void);
void lxl_pwm_on(void);
void lxl_pwm_ctrl(uint8_t ch1,uint16_t value1, uint8_t ch2,uint16_t value2);

static void pwm_light_init(void)
{
	pwm_light_ch1.pwmN=PWM_CH0;
	pwm_light_ch1.pwmPin=P3;
	pwm_light_ch1.pwmDiv=PWM_CLK_DIV_2;
	pwm_light_ch1.pwmPolarity=PWM_POLARITY_FALLING;
	pwm_light_ch1.cntTopVal = 255;	
	pwm_light_ch1.cmpVal = 0;
	hal_pwm_ch_start_config(pwm_light_ch1);

	pwm_light_ch2.pwmN=PWM_CH1;
	pwm_light_ch2.pwmPin=P2;
	pwm_light_ch2.pwmDiv=PWM_CLK_DIV_2;
	pwm_light_ch2.pwmPolarity=PWM_POLARITY_RISING;
	pwm_light_ch2.cntTopVal = 255;	
	pwm_light_ch2.cmpVal = 0;
	hal_pwm_ch_start_config(pwm_light_ch2);	
}
void set_light_model(uint8_t power,uint8_t model,uint8_t brightness,uint8_t Color_temperature)
{
	switch(power)
	{
		case LIGHT_POWER_ON:			
			{
				...
				if(model==0x0A)
				{
						...
						white_value = brightness* Color_temperature/100;
						yellow_value=brightness-white_value;					
						pwm_light_ch1.cmpVal = 255-white_value;		
						pwm_light_ch2.cmpVal = yellow_value;
						hal_pwm_ch_start_config(pwm_light_ch1);			
						hal_pwm_ch_start_config(pwm_light_ch2);
						LOG("white_value:%d  yellow_value %d\r\n",white_value,yellow_value);
				}
				else if(model==0x0B)
				{
					pwm_light_ch1.cmpVal = brightness;
					pwm_light_ch2.cmpVal = Color_temperature;
					hal_pwm_ch_start_config(pwm_light_ch1);			
					hal_pwm_ch_start_config(pwm_light_ch2);
				}					
				break;
...
void gpio_door_init(void)
{
	hal_gpio_fmux(GPIO_RED,Bit_ENABLE) ;
	hal_gpio_pin_init(GPIO_RED,OEN);
	hal_gpioretention_register(GPIO_RED);
	hal_gpio_write(GPIO_RED,0) ;

	hal_gpio_fmux(GPIO_BLUE,Bit_ENABLE) ;
	hal_gpio_pin_init(GPIO_BLUE,OEN);
	hal_gpioretention_register(GPIO_BLUE);
	hal_gpio_write(GPIO_BLUE,0) ;
	
	door_gpio_interrupt(DOOR_STATE);
	door_gpio_interrupt(RESET);	
	door_gpio_interrupt(tamper_door);
}
...
void res_network_event_handler(GPIO_Pin_e pin,IO_Wakeup_Pol_e type)
{
    ...
    { 			
			case RESET:
				updateAdvData_door(1);							
				break;
			
			case DOOR_STATE:
				updateAdvData_door(2);
				break;
			
			case tamper_door:
				updateAdvData_door(3);
				break;
...
void read_gpioSTATE(void)
{
	advertData[13]=hal_gpio_read(RESET);
	advertData[14]=hal_gpio_read(DOOR_STATE);
	advertData[15]=hal_gpio_read(tamper_door);
	
	if(!advertData[14]) hal_gpio_write(GPIO_BLUE,1);	
	else hal_gpio_write(GPIO_BLUE,0);
	
	if(advertData[15]) hal_gpio_write(GPIO_RED,1);	
	else hal_gpio_write(GPIO_RED,0);
}
static void updateAdvData_door(uint8_t id)
{
	read_gpioSTATE();	
	...
	if(id==1)
	{		
		hal_gpio_write(GPIO_BLUE,1);
		hal_gpio_write(GPIO_RED,1);
